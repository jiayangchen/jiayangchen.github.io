...
值得注意的是，本地事务提交前必须先向服务端注册分支，这个注册分支的过程会包含对全局锁的查询 —— 待提交的这些行记录的主键是否正在被其他全局事务锁定，如果是则需要循环等待，知道其他全局事务释放锁之后，该本地事务才能提交，Seata 以这样的机制保证全局事务间的写隔离。

#### 详解二阶段提交流程

![](http://ww1.sinaimg.cn/large/c3beb895gy1g4lw5e51oqj20kv0c90ta.jpg)

对服务端来说，等到一阶段完成，全局事务的发起方会向服务端申请提交这个全局事务，服务端根据 xid 查询出该全局事务后加锁，将该事务关闭使得后续分支不能再注册上来，并同时将其状态从 Begin 修改为 Committing。紧接着，判断如果该全局事务下的分支类型均为 AT 类型，则服务端会进行**异步提交**。服务端仅仅修改全局事务状态为 AsyncCommitting，然后有一个定时线程池去存储介质（文件或者数据库）中查询出待提交的全局事务日志进行提交。
...
一旦客户端提交失败或者 RPC 超时，则服务端会将该全局事务状态置位 CommitRetrying，之后会由另一个定时线程池 retryCommitting 去一直重试这些事务直至成功。

#### 详解二阶段回滚流程

![](http://ww1.sinaimg.cn/large/c3beb895gy1g4lwjsm9lrj20mj06pmxc.jpg)

回滚相对复杂一些，如果发起方一阶段抛异常，会向服务端请求回滚该全局事务，服务端会根据 xid 查询出这个全局事务，加锁关闭事务使得后续不会再有分支注册上来，并同时更改其状态 Begin 为 Rollbacking，接着进行**同步回滚**以此保证数据一致性。除了同步回滚这个点外，整个流程同提交时相似。

![](http://ww1.sinaimg.cn/large/c3beb895gy1g4lwvpn3czj20nn0cjgm8.jpg)

客户端接收到服务端的 branch rollback 请求，先根据 resourceId 拿到对应的数据源代理，然后根据 xid 和 branchId 查询出 UndoLog 记录，反序列化其中的 rollback 字段拿到数据的前后快照，根据具体 SQL 类型生成对应的 UndoExecutor，校验一下数据前后快照是否一致、或者前置快照和当前数据（这里需要 SELECT 一次）是否一致，如果一致说明不需要做回滚操作，如果不一致则生成反向 SQL 进行补偿，成功之后删除本地 UndoLog 记录。

#### 本节小结

我们通过流程图分析了一下 Seata AT 模式两阶段的工作流程，这里提一句，关于 `AT` 的工作流程，官方文档提供了一个非常易懂的例子 —— [AT 模式工作机制](https://github.com/seata/seata/wiki/AT-Mode#%E5%B7%A5%E4%BD%9C%E6%9C%BA%E5%88%B6)。
